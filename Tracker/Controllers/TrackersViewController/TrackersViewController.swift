...
final class TrackersViewController: UIViewController {
    ...
    private lazy var notFoundLabel: UILabel = {
        let label = UILabel()
        label.text = String.localized("traсkers.notFound.label")
        label.font = .systemFont(ofSize: 12, weight: .medium)
        label.numberOfLines = 2
        label.textAlignment = .center
        label.textColor = .ypBlack
    ...
    private lazy var searchTextField: UISearchTextField = {
        ...
        textField.placeholder = String.localized("traсkers.searchBar.placeholder")
        if traitCollection.userInterfaceStyle == .dark {
            textField.attributedPlaceholder = NSAttributedString(
                string: String.localized("traсkers.searchBar.placeholder"),
    ...
    private lazy var datePicker: UIDatePicker = {
        ...
        picker.widthAnchor.constraint(equalToConstant: 97).isActive = true
        return picker
    }()
    lazy var trackersCollectionView: UICollectionView = {
        ...
        collectionView.register(SupplementaryTrackersView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "header")
        return collectionView
    }()
    private lazy var filterButton: UIButton = {
        let button = UIButton()
        button.backgroundColor = .ypBlue
        button.setTitle(String.localized("traсkers.buttonFilters.title"), for: .normal)
    ...
    private func showEditingViewController(selectedTracker: Tracker, categoryTitle: String, daysCounter: String) {
        ...
        viewController.eventMode = selectedTracker.eventDate != nil
        viewController.categoryTitle = categoryTitle
        viewController.editingTracker = selectedTracker
        viewController.daysCounter = daysCounter
...
extension TrackersViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        guard let view = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "header", for: indexPath) as? SupplementaryTrackersView else { return UICollectionReusableView() }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? TrackersCell else { return UICollectionViewCell() }
        let category = visibleCategories[indexPath.section]
        let tracker = category.trackers[indexPath.row]
        cell.delegate = self
        let completedDays = viewModel.completedDays(for: tracker.id)
        cell.configCell(with: tracker, category: category.title, isCompletedToday: completedDays.completed, completedDays: completedDays.number, indexPath: indexPath)
        return cell
    }
}
...
extension TrackersViewController: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemsAt indexPaths: [IndexPath], point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        let cell = collectionView.cellForItem(at: indexPaths[0]) as? TrackersCell
        
        return UIContextMenuConfiguration(actionProvider:  { _ in
            let title = selectedTrackerCategory.title == "Закрепленные" ? String.localized("traсkers.unpin") : String.localized("traсkers.pin")
...
extension TrackersViewController: AddTrackerViewControllerDelegate, NewTrackerViewControllerDelegate {
...
extension TrackersViewController {
    ...
    private func createNavigationBar() {
        ...
        navigationBar.topItem?.title = String.localized("traсkers.navigationBar.title")
